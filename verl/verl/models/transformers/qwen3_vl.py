...
def patch_qwen3_vl_deepstack_process():
    """
    Monkey patch Qwen3-VL deepstack process to avoid autograd error:
    "Output ... is a view and is being modified inplace".

    Root cause: hidden_states may be a view produced in a custom/autograd-sensitive path,
    and in-place write `hidden_states[mask] = ...` is disallowed. We clone hidden_states
    before assignment to make the write out-of-place from autograd's perspective.
    """
    try:
    ...
    def _patched_deepstack_process(self, hidden_states: torch.Tensor, visual_pos_masks: torch.Tensor, visual_embeds: torch.Tensor):
    ...
    Qwen3VLMoeTextModel._deepstack_process = _patched_deepstack_process
    logger.info("Monkey patched Qwen3-VL deepstack process to avoid view+inplace autograd errors")
