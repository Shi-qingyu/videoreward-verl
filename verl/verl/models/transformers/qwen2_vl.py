...
def process_position_ids(position_ids: torch.Tensor) -> torch.Tensor:
    if position_ids.ndim != 3 or position_ids.size(0) not in (3, 4):
        ...
        raise ValueError("position_ids should be a 3D tensor of shape (3 or 4, batch_size, seq_length).")

    if position_ids.size(0) == 4 and is_transformers_version_in_range(max_version="4.53.3"):
