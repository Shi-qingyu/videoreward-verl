...
def compute_grpo_outcome_advantage(
    ...
    response_mask: torch.Tensor,
    index: np.ndarray,
    rollout_ignore_mask: Optional[torch.Tensor] = None,
    epsilon: float = 1e-6,
    ...
    Args:
        ...
        rollout_ignore_mask: `(Optional[torch.Tensor])`
            kept for compatibility with RL trainer paths that pass per-sample ignore masks
        epsilon: `(float)`
    ...
    with torch.no_grad():
        bsz = scores.shape[0]
        # 先把 exceed 样本从组内统计里排除，避免它们污染同组正常样本的均值/方差。
        ignore_mask = rollout_ignore_mask.bool() if rollout_ignore_mask is not None else None
        for i in range(bsz):
            if ignore_mask is not None and ignore_mask[i].item():
                continue
        ...
        for i in range(bsz):
            if ignore_mask is not None and ignore_mask[i].item():
                # 被 ignore 的样本 advantage 直接置零，后面再配合 actor 侧 response_mask 清零。
                scores[i] = 0.0
                continue
