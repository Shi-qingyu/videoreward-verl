...
def process_question_template_openqa(row):
    return QUESTION_TEMPLATE_OPENQA.format(question=row["problem"])
...
def process_question_template_mcqa(row):
    return QUESTION_TEMPLATE_MCQA.format(question=row["problem"])
...
def process_question_template_tg(row):
    return QUESTION_TEMPLATE_TG.format(event=row["problem"])
...
def process_question_template_mcqa_raw(row):
    return QUESTION_TEMPLATE_MCQA_RAW.format(question=row["question"], options=str(row["options"]))
...
def process_question_template_mcqa_think(row):
    return QUESTION_TEMPLATE_MCQA_THINK.format(question=row["question"], options=str(row["options"]))
...
def process_question_template_multi_round(row):
    return QUESTION_TEMPLATE_MULTI_ROUND.format(question=row["question"], options=str(row["options"]))
...
def process_question_template_multi_round_mc(row):
    return QUESTION_TEMPLATE_MULTI_ROUND_MC.format(question=row["question"], options=str(row["options"]))


SYSTEM_PROMPT_MAP: Dict[str, str] = {
    "default": SYSTEM_PROMPT_IMAGE_CROP,
...
def get_system_prompt(row):
    ...
    if data_source not in SYSTEM_PROMPT_MAP:
        raise KeyError(f"Unsupported data_source for system prompt: {data_source}")
...
def process_problem_with_data_source(row) -> str:
    ...
    if data_source not in DATA_SOURCE_TEMPLATES_FUNCTION:
        raise KeyError(f"Unsupported data_source for question prompt: {data_source}")
