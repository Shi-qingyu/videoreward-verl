...
def _sanitize_mm_limit_config(limit_cfg: Any, default: Dict[str, int]) -> Dict[str, int]:
    if limit_cfg is None:
        return dict(default)

    if isinstance(limit_cfg, DictConfig):
        limit_cfg = OmegaConf.to_container(limit_cfg, resolve=True)
    ...
    return sanitized or dict(default)


def _get_video_num_frames(video: Any) -> int:
    if isinstance(video, (torch.Tensor, np.ndarray)):
        return int(video.shape[0])
    return int(len(video))


def _normalize_video_fps_list(video_list: List[Any], fps_list: List[Any], default_fps: float = 2.0) -> List[float]:
    normalized = [float(fps) for fps in fps_list if fps is not None]
    if len(normalized) < len(video_list):
        fill_value = normalized[-1] if normalized else default_fps
        normalized.extend([fill_value] * (len(video_list) - len(normalized)))
    return normalized[: len(video_list)]


def _build_vllm_video_item(video: Any, fps: Optional[float]) -> tuple[Any, Dict[str, Any]]:
    num_frames = _get_video_num_frames(video)
    resolved_fps = float(fps) if fps is not None else 2.0
    metadata = {
        "fps": resolved_fps,
        "duration": num_frames / max(resolved_fps, 1e-6),
...
def _build_vllm_multi_modal_data(multi_modal_data: Dict[str, Any], video_fps_used_list: List[Any]) -> Dict[str, Any]:
    vllm_multi_modal_data = deepcopy(multi_modal_data)
    video_list = list(vllm_multi_modal_data.get("video", []))
    normalized_fps_list = _normalize_video_fps_list(video_list, video_fps_used_list)
    vllm_multi_modal_data["video"] = [
        _build_vllm_video_item(video, fps) for video, fps in zip(video_list, normalized_fps_list, strict=False)
    ]
    return vllm_multi_modal_data
...
def compute_reward(reward_fn, batch, non_tensor_batch, iteration):
    ...
    def _supports_kwarg(func: Any, kwarg_name: str) -> bool:
        try:
            ...
            return False
        return (
            kwarg_name in signature.parameters
    ...
    def _scatter_sample_scores(score_list: Optional[List[Any]], reward_tensor: torch.Tensor) -> torch.Tensor:
        output = torch.zeros_like(reward_tensor, dtype=torch.float32)
        if score_list is None:
            return output

        prompt_length = int(batch["prompts"].shape[-1])
        for sample_idx, score in enumerate(score_list):
            valid_response_length = int(batch["attention_mask"][sample_idx, prompt_length:].sum().item())
            if valid_response_length > 0:
                output[sample_idx, valid_response_length - 1] = float(score)
        return output

    def _invalid_uids_from_valid_mask(valid_mask: Optional[List[Any]]) -> List[Any]:
        if valid_mask is None or "uid" not in non_tensor_batch:
            return []
        uid_values = list(non_tensor_batch["uid"])
        return [uid_values[i] for i, is_valid in enumerate(valid_mask) if not bool(is_valid)]

    # Prefer the current repo reward-manager protocol when available.
    if _supports_kwarg(reward_fn, "return_dict"):
        reward_kwargs = {"return_dict": True}
        if _supports_kwarg(reward_fn, "iteration"):
            reward_kwargs["iteration"] = iteration
        reward_result = reward_fn(cur_batch, **reward_kwargs)
    elif _supports_kwarg(reward_fn, "iteration"):
        reward_result = reward_fn(cur_batch, iteration=iteration)
    else:
        reward_result = reward_fn(cur_batch)

    if isinstance(reward_result, dict):
        reward_tensor = _ensure_tensor(reward_result["reward_tensor"])
        reward_extra_info = reward_result.get("reward_extra_info", {})
        acc_reward_tensor = _scatter_sample_scores(reward_extra_info.get("acc_scores"), reward_tensor)
        format_reward_tensor = _scatter_sample_scores(reward_extra_info.get("format_scores"), reward_tensor)
        overlong_reward_tensor = _scatter_sample_scores(reward_extra_info.get("overlong_scores"), reward_tensor)
        invalid_uids = _invalid_uids_from_valid_mask(reward_extra_info.get("valid_mask"))
    elif isinstance(reward_result, tuple):
        if len(reward_result) == 5:
            reward_tensor, acc_reward_tensor, format_reward_tensor, overlong_reward_tensor, invalid_uids = reward_result
            reward_tensor = _ensure_tensor(reward_tensor)
            acc_reward_tensor = _ensure_tensor(acc_reward_tensor)
            format_reward_tensor = _ensure_tensor(format_reward_tensor)
            overlong_reward_tensor = _ensure_tensor(overlong_reward_tensor)
        elif len(reward_result) == 4:
            reward_tensor, acc_reward_tensor, format_reward_tensor, invalid_uids = reward_result
            reward_tensor = _ensure_tensor(reward_tensor)
            acc_reward_tensor = _ensure_tensor(acc_reward_tensor)
            format_reward_tensor = _ensure_tensor(format_reward_tensor)
            overlong_reward_tensor = torch.zeros_like(reward_tensor, dtype=torch.float32)
        else:
            raise ValueError(
                f"Unsupported reward tuple shape from {type(reward_fn).__name__}: "
                f"expected 4 or 5 values, got {len(reward_result)}"
            )
    else:
        reward_tensor = _ensure_tensor(reward_result)
        acc_reward_tensor = torch.zeros_like(reward_tensor, dtype=torch.float32)
        format_reward_tensor = torch.zeros_like(reward_tensor, dtype=torch.float32)
        overlong_reward_tensor = torch.zeros_like(reward_tensor, dtype=torch.float32)
        invalid_uids = []
...
class AsyncvLLMEngine:
    ...
    def __init__(
        ...
        self.free_cache_engine = bool(self.config.rollout.get("free_cache_engine", True))
        self.max_total_response_length = self.config.rollout.max_total_response_length

        self.ignore_exceed = self.config.actor.get("ignore_exceed", False)
        ...
        if self.reward_fn is None or self.val_reward_fn is None:
            if reward_config is None:
                raise ValueError("reward_config must be provided when reward managers are not passed to AsyncvLLMEngine.")

            reward_config_obj = OmegaConf.create(reward_config)
            reward_kwargs = reward_config_obj.reward_model.get("reward_kwargs", {})
            reward_kwargs = OmegaConf.to_container(OmegaConf.create(reward_kwargs), resolve=True)
    ...
    def init_engine(self):
        ...
        if max_model_len is None:
            max_total = config.get("max_total_response_length", None)
            if max_total is not None:
                max_model_len = int(max_total)
            else:
                max_model_len = int(config.prompt_length) + int(config.response_length)
        else:
            max_model_len = int(max_model_len)
        print(f"[AsyncvLLMEngine] max_model_len={max_model_len}")
        ...
        mm_limit_cfg = _sanitize_mm_limit_config(config.get("limit_mm_per_prompt"), {"video": 1})
        ...
        engine_args = AsyncEngineArgs(
            ...
            limit_mm_per_prompt=mm_limit_cfg,
        )
    ...
    def calculate_video_token_num(self, video):
        video_inputs = self.processor.video_processor([video], do_sample_frames=False)
        grid_thw = video_inputs['video_grid_thw'][0]
        video_token_num = int(grid_thw.prod().item() // self.merge_length)
        return video_token_num

    def post_process_single(self, prefix_length, vllm_input, multi_turn_response_mask, request_multi_modal_data=None):
        ...
        video_grid_thws = []
        extra_videos = []
        if request_multi_modal_data and 'video' in request_multi_modal_data:
            extra_videos = list(request_multi_modal_data['video'][1:])
        elif 'video' in vllm_input['multi_modal_data']:
            extra_videos = [
                video_item[0] if isinstance(video_item, tuple) and len(video_item) == 2 else video_item
                for video_item in vllm_input['multi_modal_data']['video'][1:]
            ]

        if extra_videos:
            processed_video_inputs = self.processor.video_processor(extra_videos, do_sample_frames=False)
    ...
    def preprocess_for_reward_function(self, input_ids, attention_mask, response, response_masks):
        ...
        # all should be (B*R, self.max_total_response_length)
        if response.shape[1] > self.max_total_response_length:
            response = response[:,:self.max_total_response_length]
            response_generation_mask = response_generation_mask[:,:self.max_total_response_length]
        elif response.shape[1] < self.max_total_response_length:
            response = pad_sequence_to_length(response, self.max_total_response_length, self.pad_token_id)
            response_generation_mask = pad_sequence_to_length(response_generation_mask, self.max_total_response_length, 0)
    ...
    @staticmethod
    def _is_vllm_prompt_length_error(exc: Exception) -> bool:
        # 只把 vLLM 的模长校验异常识别成 exceed；其他异常仍然向上抛，避免误吞真实 bug。
        if not isinstance(exc, ValueError):
            return False

        message = str(exc)
        return "maximum model length" in message and "prompt" in message

    def _finalize_async_rollout_request(
        self,
        _req,
        *,
        prefix_length,
        ...
        # 无论是正常结束还是 overlength 兜底，都统一走这里收尾，保证下游拿到一致的字段结构。
        assistant_turn_num = sum(int(mask.any().item()) for mask in multi_turn_response_mask[1:])
        total_response_tokens = sum(int(mask.sum().item()) for mask in multi_turn_response_mask[1:])
        avg_response_tokens_per_turn = (
            total_response_tokens / assistant_turn_num if assistant_turn_num > 0 else 0.0
        )

        if len(multi_turn_response_mask) > 1:
            all_response, all_response_masks = self.post_process_single(
                prefix_length,
                vllm_input,
                multi_turn_response_mask,
                request_multi_modal_data=multi_modal_data,
            )
        else:
            device = attention_mask.device
            all_response = torch.empty(0, dtype=torch.int64, device=device)
            all_response_masks = torch.empty(0, dtype=torch.int64, device=device)

        input_ids, response, seq, attention_mask, multi_turn_response_mask = self.preprocess_for_reward_function(
            input_ids,
            attention_mask,
            all_response,
            all_response_masks,
        ...
        non_tensor_batch_cur = {
            'data_source': np.array([_req.data_source], dtype=object),
            'ground_truth': np.array([_req.ground_truth], dtype=object),
            'raw_prompt': np.array([_req.raw_prompt], dtype=object),
            'uid': np.array([_req.uid], dtype=object),
        }

        if is_validate:
            reward_tensor_cur, acc_reward_tensor_cur, format_reward_tensor_cur, overlong_reward_tensor_cur, invalid_uids_cur = compute_reward(
                self.val_reward_fn,
                batch_cur.to('cpu'),
                non_tensor_batch_cur,
                current_iteration,
        ...
        _req.multi_modal_data = multi_modal_data if _req.multi_modal_data else _req.multi_modal_data
        ...
        _req.round_num = assistant_turn_num
        _req.void = void
    ...
    def process_tool_call(
        ...
        if tool_success:
            ...
            request_multi_modal_data['video'].append(video_clip)
            video_fps_used_list.append(tool_outputs['fps'])
            vllm_input['multi_modal_data']['video'].append(_build_vllm_video_item(video_clip, tool_outputs['fps']))
    ...
    async def _async_rollout_a_request(
        ...
        if _req.multi_modal_data:
            multi_modal_data = deepcopy(_req.multi_modal_data)
            ...
            vllm_input = {
                ...
                'multi_modal_data': _build_vllm_multi_modal_data(multi_modal_data, video_fps_used_list),
            }
        ...
        while current_iteration < max_iterations:
            ...
            try:
                ...
                if self._is_vllm_prompt_length_error(exc):
                    # 下一轮请求在进入 vLLM 前就因真实多模态长度超限被拒绝；
                    # 这里降级成 exceed，让 trainer/actor 走 ignore_exceed 链路，而不是整批崩掉。
                    print(f"[AsyncvLLMEngine] request {_req.request_id} exceeded max model length: {exc}")
                    exceed = True
                    break
                raise
            ...
            if len(tool_call_contents) > 0:
                ...
                if context_length >= self.max_total_response_length - 2000:
                    exceed = True
                    break
        ...
        return self._finalize_async_rollout_request(
            _req,
            prefix_length=prefix_length,
            vllm_input=vllm_input,
            multi_turn_response_mask=multi_turn_response_mask,
            multi_modal_data=multi_modal_data,
            video_fps_used_list=video_fps_used_list,
            input_ids=input_ids,
            attention_mask=attention_mask,
    ...
    def post_process(self, prompts: DataProto, output_req_list: List[AsyncRolloutRequest]) -> DataProto:
        ...
        if position_ids.dim() == 3:  # qwen2vl mrope
            ...
            for prompt_with_response, attn_mask, multi_modal_data in zip(seq, attention_mask, multi_modal_data_list):
                video_inputs = self.processor.video_processor(multi_modal_data['video'], do_sample_frames=False)
