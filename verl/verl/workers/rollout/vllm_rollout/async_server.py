...
class AsyncLLMEngineManager:
    ...
    def _get_worker_node_ids(self) -> List[str]:
        register_center_name = f"{self.worker_group.name_prefix}_register_center"
        try:
            register_center = ray.get_actor(register_center_name)
            worker_node_ids = ray.get(register_center.get_worker_info.remote())
        except ValueError:
            logger.warning(
                "Named actor %s is unavailable; falling back to querying worker node IDs directly.",
        ...
        assert len(worker_node_ids) == self.worker_group.world_size
    ...
    def __init__(
        ...
        *,
        ...
        worker_node_ids = self._get_worker_node_ids()

        self.async_llm_servers = [None] * self.rollout_dp_size

        data_mode = os.getenv("DATA_MODE", "image")
        if data_mode == "video":
            from verl.workers.rollout.vllm_rollout.vllm_async_engine_video import AsyncvLLMEngine
            print("The data mode is video, using vllm_async_engine_video!")
        else:
            raise ValueError(f"Legacy vllm_multi_turn_tool_call async manager only supports DATA_MODE=video, got {data_mode}")
        
        engine_class = AsyncvLLMEngine
        if not config.rollout.max_model_len:
            max_total = config.rollout.get("max_total_response_length", None)
            if max_total:
                config.rollout.max_model_len = int(max_total)
            else:
                config.rollout.max_model_len = int(config.rollout.prompt_length) + int(config.rollout.response_length)
            ...
            servers = {
                rollout_dp_rank: engine_class.options(
                    ...
                    scheduling_strategy=ray.util.scheduling_strategies.NodeAffinitySchedulingStrategy(
                        node_id=worker_node_ids[rollout_dp_rank * self.rollout_tp_size],
                        soft=False,
                    ),
    ...
    def generate_sequences(self, prompts: DataProto, **sampling_params) -> DataProto:
        ...
        for i, server in enumerate(self.async_llm_servers):
            shard_indices = indices[i * chunk_size : (i + 1) * chunk_size]
            if len(shard_indices) == 0:
                continue
            result.append(
                server.generate_sequences.remote(
                    prompts.select_idxs(shard_indices), **sampling_params
                )
            )
