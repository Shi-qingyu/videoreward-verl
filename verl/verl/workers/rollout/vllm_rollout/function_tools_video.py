...
try:
    from my_qwen_vl_utils.video_o3_vision_process import (
        FPS_MAX_FRAMES, IMAGE_FACTOR, crop_video_raw, fetch_video_raw, fetch_video_raw_frame, resample_video_from_raw,
    )
...
def crop_video(raw_video, raw_fps, temporal_segment, sampling_strategy, frames_sample_fps=2.0):
    ...
    if video_path.endswith(".mp4"):
        # Re-read the temporal segment from disk at original fps, instead of
        # cropping from the pre-loaded downsampled tensor (whose fps is too low
        # to resolve short time ranges).
        crop_video_ele = {
            "video": video_path,
