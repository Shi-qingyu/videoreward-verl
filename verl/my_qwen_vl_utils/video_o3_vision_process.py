...
def _detect_image_factor() -> int:
    """Auto-detect IMAGE_FACTOR from model config.json.
    Qwen2.5-VL (model_type in qwen2_vl/qwen2_5_vl) uses 28, Qwen3-VL and others use 32.
    Can be overridden via IMAGE_FACTOR env var.
    """
    explicit = os.environ.get("IMAGE_FACTOR")
    if explicit is not None:
        return int(explicit)
    model_path = os.environ.get("PRETRAINED_PATH", "")
    if model_path:
        import json
        ...
        try:
            ...
            logger.warning(f"Cannot read {config_path}: {e}, defaulting IMAGE_FACTOR=28")
    return 28


IMAGE_FACTOR = _detect_image_factor()
logger.info(f"IMAGE_FACTOR={IMAGE_FACTOR} (detected from PRETRAINED_PATH={os.environ.get('PRETRAINED_PATH', '<unset>')})")
